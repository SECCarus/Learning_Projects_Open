...
function [findloc,center,p_all,theta,check_work] = template_matching_v2(raw_img,raw_cmp_img,test_mode)
    ...
    if norm(sort_X(end) - sort_X(end - 1)) > norm(sort_X(end-1) - sort_X(end - 2))*4
        max_x = mean(sort_X(end-1) - sort_X(end - 2));
    else
        max_x = sort_X(end);
    end

    if norm(sort_Y(end) - sort_Y(end - 1)) > norm(sort_Y(end-1) - sort_Y(end - 2))*4
        max_y = mean(sort_Y(end-1) - sort_Y(end - 2));
    else
        max_y = sort_Y(end);
    end
    min_x = min([sort_X(1),sort_X(2)]);
    min_y = min([sort_Y(1),sort_Y(2)]);
    ...
    % % 判断是否检测到图片
    if  center(1) == -720 | center(2) == -720 | isnan(center(1)*center(2))
        findloc = [0 0 0 0];
        check_work = 0;
    else
        % findloc 指定为 [x y w h]
