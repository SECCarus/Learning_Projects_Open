...
% % 目标函数（电车和电池调度仿真模型）
function [static,s2_resource] = cal_obj_val_based_on_simulation_model(xj,Tj,k,time_long,PD,SS1,threshold,max_car_num,max_battery_num,if_draw,if_show)
...
%     s2_resource.fully_soc_car = max_car_num - length(find(xj > 0)) - s2_resource.fully_soc_car;
%     s2_resource.empty_soc_car = 0;
%     s2_resource.wait_restart_car = 0; % 属于空电车辆，但属于原定行驶车辆
%     s2_resource.wait_exchange_good_car = 0; % 准备更换货物的车辆
%     s2_resource.wait_exchange_car = 0 ; % 准备空载换车的车辆
...
%     s2_resource.charging_bty = 0;  
%     s2_resource.empty_bty = 0;
%     s2_resource.fully_bty = (max_battery_num - max_car_num * 6) - s2_resource.fully_bty;
    
    % 电池组模6调整
%     ret = mod(s2_resource.fully_bty,6);
%     if ret ~= 0
...
%     end
    ...
    % % 统计量
    static = struct();
    static.load_num = 0; % 累计装卸货次数
    static.max_charging_piles_used_num = 0; % 充电桩最大使用量
    static.max_wait_start_car_num = 0;
    static.s2_wait_start_car_num = 0;
    static.s2_charged_finish_times = 0; % s2 累计换电站充电完成数
    static.xj_start = xj;
    static.Tj_start = Tj;
    static.end_time = 1; % 终止时间
    static.total_fully_soc_car = s2_resource.fully_soc_car; % 满电待机车辆
    static.total_empty_soc_car = s2_resource.empty_soc_car; % 空点车辆
    static.total_wait_restart_car = s2_resource.wait_restart_car; % 等待重启车辆
    static.total_wait_exchange_good_car = s2_resource.wait_exchange_good_car; % 等待交换货物的车辆
    static.total_wait_exchange_car = s2_resource.wait_exchange_car; % 等待交换的车辆
    static.total_wait_replace_bty = s2_resource.wait_replace_bty; % 等待从车辆上更换下来的电池
    static.total_charging_bty = s2_resource.charging_bty; % 正在充电电池
    static.total_empty_bty = s2_resource.empty_bty ; % 空电电池
    static.total_fully_bty = s2_resource.fully_bty; % 满电电池
    ...
    static.accumulate_restart_car_num = 0; % 累计车辆重启次数
    static.accumulate_charged_finish_time = static.s2_charged_finish_times;% 累计换电站充电完成数
    static.accumulate_exchange_good_num = 0; % 累计交换货物次数（满载换车）
    static.accumulate_exchange_car_num = 0; % 累计空载换车次数
    static.accumulate_replace_bty_num = 0; % 累计更换电池组次数
    
    static.load_num_list = zeros(1,time_long);                  % 存储每次循环的卸货数
    static.charging_bty_list = zeros(1,time_long);              % 存储每次循环的换电池数
    static.car_num_list = zeros(1,time_long);                   % 存储当前行驶车辆的总数
...
%             end
            ...
            if ~isempty(id)
                sel = 0;
                if s2_resource.fully_bty >= (s2_resource.fully_soc_car * 6)
                    sel = 1; % 电池资源较多
                elseif s2_resource.fully_bty < (s2_resource.fully_soc_car * 6)
                    sel = 2; % 空车资源较多
                end
            end
            ...
            if ~isempty(id) && (sel == 2)
                ...
                if s2_resource.fully_soc_car >= length(id) 
                    % 更新参数
                    s2_resource.fully_soc_car = s2_resource.fully_soc_car - length(id);
                    s2_resource.wait_exchange_good_car = s2_resource.wait_exchange_good_car + length(id);
                ...
                elseif (s2_resource.fully_bty >= (length(id) - s2_resource.fully_soc_car) * 6) 
                    ...
                    if s2_resource.fully_soc_car > 0
                        sub_id = id(1:s2_resource.fully_soc_car);
                        s2_resource.fully_soc_car = s2_resource.fully_soc_car - length(sub_id);
                        s2_resource.wait_exchange_good_car = s2_resource.wait_exchange_good_car + length(sub_id);
            ...
            if ~isempty(id) && (sel == 1) % 电池资源占优
                % 如果电池资源充足
                if s2_resource.fully_bty >= (length(id) * 6)
                    s2_resource.fully_bty = s2_resource.fully_bty - length(id) * 6;
                    s2_resource.wait_replace_bty = s2_resource.wait_replace_bty + length(id) * 6;

                    on_replace_bty(id) = replace_bty_time;
                    need_replace_bty_id = unique([need_replace_bty_id,id]); 
                % 电池资源不足，但满电车辆资源充足
                elseif (s2_resource.fully_soc_car * 6 >= (length(id) * 6 - s2_resource.fully_bty))

                    ptr = 0;
                    if s2_resource.fully_bty > 0
                        sub_id = id(1:floor(s2_resource.fully_bty/6));
                        s2_resource.fully_bty = s2_resource.fully_bty - length(sub_id) * 6;
                        s2_resource.wait_replace_bty = s2_resource.wait_replace_bty + length(sub_id) * 6;    
            ...
            if ~isempty(id)
                ...
                if  ret2 == 0 || ret1 == 0
                    [s2_resource] = assign_wait_restart_car(s2_resource,sub_record);
                    ...
                    static.accumulate_restart_car_num = static.accumulate_restart_car_num + length(id);
                end
            end
            ...
            if ~isempty(id)
                ...
                [s2_resource] = assign_replace_bty(s2_resource,sub_record);
                record_on_bty_strategy(need_replace_bty_id(id)) = 0;
                soc_k(need_replace_bty_id(id)) = 1;
                need_replace_bty_id(id) = [];
                static.accumulate_replace_bty_num = static.accumulate_replace_bty_num + length(id);
            end
            ...
            if ~isempty(id)
                ...
                [s2_resource] = assign_exchange_good(s2_resource,sub_record);
                record_on_bty_strategy(need_exchange_good_id(id)) = 0;
                soc_k(need_exchange_good_id(id)) = 1;
                need_exchange_good_id(id) = [];
                static.accumulate_exchange_good_num = static.accumulate_exchange_good_num + length(id);
            end
            ...
            if ~isempty(id)
                ...
                [s2_resource] = assign_exchange_car(s2_resource,sub_record);
                record_on_bty_strategy(need_exchange_car_id(id)) = 0;
                soc_k(need_exchange_car_id(id)) = 1;
                need_exchange_car_id(id) = [];
                static.accumulate_exchange_car_num = static.accumulate_exchange_car_num + length(id);
            end
            ...
            if ~isempty(id)
                ...
                [s2_resource,static] = assign_charging_finish_bty(s2_resource,sub_record,static);
                record_on_charging_pile(need_wait_charging_pile_finish_id(id)) = 0;
                need_wait_charging_pile_finish_id(id) = [];
                static.accumulate_charged_finish_time = static.accumulate_charged_finish_time + length(id);
            end
        ...
        if static.max_wait_start_car_num < s2_resource.wait_restart_car
            static.max_wait_start_car_num = s2_resource.wait_restart_car;
        end
...
%         end
        ...
        if if_draw
            t = 0.2*i;                                    % 实际时间（分钟）
            dist = mod(dist_k(index),20);                 % 所有车辆的位置(总路程对20取余)
            soc = soc_k(Tj == 1)*100;                     % 第一辆车的电量

            % 绘制公路
            plot([0,20],[-0.5,-0.5],'k',[0,20],[0.5,0.5],'k','linewidth',1.5),hold on,ylim([-10,10]);
            ...
            x_D = 14.6;
            x_S2_2 = 20;
            plot(x_S2_1,0,'bo',x_P,0,'ro',x_S1,0,'co',x_D,0,'mo',x_S2_2,0,'bo','linewidth',1.5),hold on;
            ...
            text(x_D,1.5,'D')
            text(x_S2_2,1.5,'S2')
            xlabel(['当前总卸货数',num2str(static.load_num),',正在充电的电池数',num2str(static.total_charging_bty),'第一辆车的电量',num2str(soc),'%']);

            set(gcf,'position',[0 0 1500 820]);         % 大屏显示
            set(gcf,'menubar','none')                   % 去掉figure菜单栏
            set( gca, 'XTick', [0:2:20], 'YTick', [] );
    %         set(get(handle(gcf),'JavaFrame'),'Maximized',true)    %全屏展示
        ...
        static.load_num_list(i) = static.load_num;
        static.charging_bty_list(i) = static.total_charging_bty;
        static.car_num_list(i) = size(index,2);
    ...
    % 载货量的时序图
    figure(1)
    plot([1:time_long/600],static.load_num_list(1:time_long/600),'linewidth',1.5,'color','[0.00,0.45,0.74]');xlabel('时间/5mins');ylabel('卸货总数/次')
    
    % 充电电池数的时序图
    figure(2)
    plot([1:time_long/100],static.charging_bty_list(1:time_long/100),'linewidth',1.5,'color','[0.85,0.33,0.10]');xlabel('时间/5mins');ylabel('正在充电的电池数/个')
    figure(3)
    plot([(199/200)*time_long:time_long],static.charging_bty_list((199/200)*time_long:time_long),'linewidth',1.5,'color','[0.85,0.33,0.10]');xlabel('时间/5mins');ylabel('正在充电的电池数/个')

    %车辆总数的时序图 
    figure(4)
    plot([1:time_long/600],static.car_num_list(1:time_long/600),'linewidth',1.5,'color','[0.93,0.69,0.13]');xlabel('时间/5mins');ylabel('工作车的数量/辆');
...
% % 配置更换后的电池资源
function [s2_resource] = assign_replace_bty(s2_resource,sub_record)
    
    % 换电站1
    s1 = find((sub_record == 1));
    s2_resource.empty_bty = s2_resource.empty_bty + length(s1) * 6;
    s2_resource.wait_replace_bty = s2_resource.wait_replace_bty - length(s1) * 6;
...
function [s2_resource] = assign_exchange_good(s2_resource,sub_record)
    
    % 换电站1
    % 车辆交换货物完成
    s1 = find((sub_record == 1));
    s2_resource.empty_soc_car = s2_resource.empty_soc_car + length(s1);
    s2_resource.wait_exchange_good_car = s2_resource.wait_exchange_good_car - length(s1);
...
function [s2_resource] = assign_exchange_car(s2_resource,sub_record)

    % 换电站1
    % 空载车辆交换完成
    s1 = find((sub_record == 1));
    s2_resource.empty_soc_car = s2_resource.empty_soc_car + length(s1);
    s2_resource.wait_exchange_car = s2_resource.wait_exchange_car - length(s1);
...
function [s2_resource] = assign_wait_restart_car(s2_resource,sub_record)

    % 换电站1
    % 空载车辆交换完成
    s1 = find((sub_record == 1));
    s2_resource.wait_restart_car = s2_resource.wait_restart_car - length(s1);
...
function [s2_resource,static] = assign_charging_finish_bty(s2_resource,sub_record,static)
    
    % 位于换电站1
    s1 = find((sub_record == 1));
    s2_resource.fully_bty = s2_resource.fully_bty + length(s1) * 6;
    s2_resource.charging_bty = s2_resource.charging_bty - length(s1) * 6;

    static.s2_charged_finish_times = static.s2_charged_finish_times + length(s1);
