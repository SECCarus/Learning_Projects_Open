...
% % 目标函数（电车和电池调度仿真模型）
function [static,s1_resource,s2_resource] = cal_obj_val_based_on_simulation_model(xj,Tj,k,fast_charging_k,time_long,PD,SS1,threshold,max_car_num,max_battery_num,if_show)
...
%     resource_prop = length(find(xj > 0)) / max_car_num;
    ...
    static.init_fast_charging_k = fast_charging_k;

    % % 开启仿真
    i = 1;
    % S2___空载_____P_____满载______S1_____满载_____D______空载____S2
    % Xj == 1 时则于S2出发 Xj == 2时，则于S1出发
    ...
    fast_charging_num = fast_charging_k;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    while i < time_long
        ...
        if ~isempty(index)
            ...
            if s1_resource.empty_bty / 6 > 0
                ...
                % 是否安排快充
                access_fast_chaging_file_num = fast_charging_num - length(find(on_charging_pile(need_wait_charging_pile_finish_id) > 0.000001)) - length(find(on_wait_restart_car(need_wait_restart_car_id) > 0.000001));
                if access_fast_chaging_file_num > 0
                    if access_fast_chaging_file_num >= length(sub_id) 
                        on_charging_pile(sub_id) = fast_charging_time;
                    else
                        on_charging_pile(sub_id(1:access_fast_chaging_file_num)) = fast_charging_time;
                        on_charging_pile(sub_id(access_fast_chaging_file_num+1:end)) = charging_time;
                    end
                else
            ...
            if ~isempty(id)
                ...
                elseif (s2_resource.fully_bty + s2_resource.fully_soc_car * 6) < length(id) * 6
                    ...
                    if access_fast_chaging_file_num > 0
                        if access_fast_chaging_file_num >= length(sub_id) 
                            on_wait_restart_car(sub_id) = fast_charging_time;
                        else
                            on_wait_restart_car(sub_id(1:access_fast_chaging_file_num)) = fast_charging_time;
                            on_wait_restart_car(sub_id(access_fast_chaging_file_num+1:end)) = wait_restart_time;
                        end
                    else
            ...
            if ~isempty(id)
                ...
                if ret == 1 && length(find(sub_record == 1)) > 1                  
                ...
                if ret == 2 && length(find(sub_record == 2)) > 1                  
            ...
            delta_soc_k = 0.2 .* c_v2 .* car_empty + ...
                           0.2 .* c_v1 .* car_load;
        ...
        if static.max_charging_piles_used_num  < (length(find(on_charging_pile > 0)) + length(on_wait_restart_car))
            static.max_charging_piles_used_num = (length(find(on_charging_pile > 0)) + length(on_wait_restart_car)); % 统计最大充电桩使用量
        end
