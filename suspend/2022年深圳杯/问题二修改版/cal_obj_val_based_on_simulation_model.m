...
% % 目标函数（电车和电池调度仿真模型）
function [static,s1_resource,s2_resource] = cal_obj_val_based_on_simulation_model(xj,Tj,k,time_long,PD,SS1,threshold,max_car_num,max_battery_num,if_show)
    ...
    % % 资源分配策略
    resource_prop = sum(xj) / max_car_num;
    ...
    s2_resource.fully_soc_car = floor((max_car_num - sum(xj)) / 2);
    ...
    s2_resource.fully_bty = floor((max_battery_num - max_car_num * 6) / 2);

    s1_resource.fully_soc_car = max_car_num - sum(xj) - s2_resource.fully_soc_car;
    ...
    static.On_road_car = sum(xj); % 正在行驶车辆
    ...
    while i < time_long
        ...
        if ~isempty(index)
            ...
            if ~isempty(id)
                ...
                xj(need_wait_restart_car_id(id)) = 1; % 代表重新出发
        ...
        static.total_empty_bty = s1_resource.empty_bty + s2_resource.empty_bty; % 空电电池
        static.total_fully_bty = s1_resource.fully_bty + s2_resource.fully_bty; % 满电电池
        static.On_road_car = [static.On_road_car,sum(xj)]; % 正在行驶车辆数变化序列
    ...
    if if_show  
        ...
        disp(['最初派出车辆数: ',num2str(sum(static.xj_start))]);
        disp(['最终剩余派出车辆数: ',num2str(sum(static.xj_end))]);
    end
end
